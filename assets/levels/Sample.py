...
class Sample():
    def __init__(self):
        ...
        self.tiles, self.world_obj, self.entities = E.load_level_from_image(
            pg.image.load('assets/levels/level_map_0.png'))
        ...
        self.player.player_y_momentum = 0
    ...
    def player_input(self, dt, entity, collision):
        movement = [0, 0]
        self.player.player_y_momentum += 0.2
        ...
        if keys['up'] and collision['bottom']:
            if self.player.air_timer < 10:
                self.player.player_y_momentum = - 250 * dt

        movement[1] += self.player.player_y_momentum
        if self.player.player_y_momentum > 3:
    ...
    def play(self, display, dt):
        ...
        if self.player.collision['collision']['bottom'] or self.player.collision['collision']['top']:
            self.player.player_y_momentum = 1 * dt
            self.player.air_timer = 0
        else:
