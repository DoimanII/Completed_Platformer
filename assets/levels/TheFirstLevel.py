...
class TheFirstLevel():
    ...
    def play(self, display, dt, last_scene):
        ...
        #remove particle
        self.particles.extend(self.player.particles) # LAGS
        self.particles = [p for p in self.particles if p[2] > 0]
        self.player.particles = [p for p in self.player.particles if p[2] > 0]

        for particle in self.particles: # ТОЛЬКО ПАРТИКЛЫ ИГРОКА!
        ...
        for trigger in self.triggers:
            if trigger.set(self.player):
                ...
                self.change_level_to = 'TheSecond'
